...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens, (n_hidden, n_cell) = self.encoder(X)
        enc_hiddens = pad_packed_sequence(enc_hiddens)[0].permute(1, 0, 2)
        init_decoder_hidden = self.h_projection(torch.cat((n_hidden[0], n_hidden[1]), 1))
        init_decoder_cell = self.c_projection(torch.cat((n_cell[0], n_cell[1]), 1))
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, 1, dim=0):
            Ybar_t = torch.squeeze(Y_t, 0)
            Ybar_t = torch.cat((Ybar_t, o_prev), dim=1)
